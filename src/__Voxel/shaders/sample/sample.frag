...
VoxelData floorData(vec3 v){
    return VoxelData(v,groundColor,vec2(1./255.,0.),groundRoughness,groundMetalness,0.,0.,1.);
}

VoxelData airData(vec3 v){
    return VoxelData(v,vec3(1.),vec2(0.),0.,0.,0.,1.,1.);
}
...
VoxelData voxelData(vec3 v){
    ...
    if(vd.index==vec2(0.))return airData(v);
...
vec3 skyColor(vec3 r0,vec3 r,float sunScale){
    ...
    if(raySphereIntersect(r0,r,lightPosition,lightRadius)>0.){
...
bool intersect(vec3 r0,vec3 r,inout VoxelData vd){
    ...
    vec3 v=vec3(0.);
    if(!inBounds(r0)){
        if(!rayAABB(r0,r,vec3(0.),bounds,tBounds)){
            if(r.y>=0.){
                return false;
            }
    ...
    vec3 stp=sign(r);
    vec3 tDelta=1./abs(r);
    vec3 tMax=step(0.,r)*(1.-fract(r0))+(1.-step(0.,r))*fract(r0);
    tMax=tMax/abs(r);
...
void main(){
    ...
    // Recover NDC
    vec2 jitter=randUniform2(randOffset)-.5;
    vec4 ndc=vec4(
        2.*(gl_FragCoord.xy+jitter)/res-1.,
        2.*gl_FragCoord.z-1.,
        1.
    );
    ...
    float ddof=dofDist*length(bounds)+length(.5*bounds-eye)-length(bounds)*.5;
    ...
    for(int b=0;b<nBounces;b++){
        ...
        if(intersect(r0,ray,vd)){
            ...
            float tVoxel=0.;
            rayAABB(r0,ray,vd.xyz,vd.xyz+1.,tVoxel);
            ...
            if(randUniform1(randOffset)<=vd.metalness){
                ...
                // nonmetallic
                const float F0=0.;
                float F=F0+(1.-F0)*pow(1.-dot(-ray,n),5.);
            ...
            if(ray==diffuse){
                ...
                if(!intersect(r0,rLight,_vd)){
                    accm+=mask*skyColor(r0,rLight,.5)*clamp(dot(rLight,m),0.,1.);
                }
            }
        }else{
            accm+=mask*skyColor(r0,ray,b==0?1.:0.).rgb;
            break;
        }
    }
